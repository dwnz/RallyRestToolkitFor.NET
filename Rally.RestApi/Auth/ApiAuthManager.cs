...
namespace Rally.RestApi.Auth
{
  ...
  public abstract class ApiAuthManager
  {
    ...
    protected ApiAuthManager(bool isUiSupported, string applicationToken, string encryptionKey, IEncryptionRoutines encryptionRoutines, string webServiceVersion = RallyRestApi.DEFAULT_WSAPI_VERSION)
    {
      if (isUiSupported)
      {
      ...
      Api = new RallyRestApi(this, webServiceVersion);
    }
      ...
      string loginFailureUnknown = null)
    {
      ...
      LoginWindowDefaultServer = loginWindowDefaultServer ?? new Uri(RallyRestApi.DEFAULT_SERVER);
      ...
      bool allowSso)
    {
      ...
      try
      {
        if (String.IsNullOrWhiteSpace(errorMessage))
        {
          Api.Authenticate(LoginDetails.Username, LoginDetails.GetPassword(), serverUri, proxy, allowSSO: allowSso);
        }
      }
